...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request,'main/login.html',{'mesg':'WRONG CREDENTIALS! Please Try Again.'})
...
def student(request):
    ...
    if flag:
        flag = False
        return render(request, 'main/student.html',{'num': [1,2,3,4,5,6,7,8],'mesg':"Submitted Successfully", 'status': status})
    elif file_error:
         file_error = False
         return render(request, 'main/student.html',{'num': [1,2,3,4,5,6,7,8], 'error_mesg':error_mesg, 'status': status})
    else:
        return render(request, 'main/student.html',{'num': [1,2,3,4,5,6,7,8], 'status': status})
